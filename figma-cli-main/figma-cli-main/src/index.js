...
import { isPatched, patchFigma, unpatchFigma, getFigmaCommand, getCdpPort, detectCdpPort } from './figma-patch.js';
...
async function daemonExec(action, data = {}) {
  ...
  try {
    ...
    throw new Error(`Daemon connection failed: ${e.message}`);
  }
}
...
function runFigmaUse(cmd, options = {}) {
  try {
    ...
    if (error.message?.includes('enableCompileCache')) {
      // Startup version check (above) already warned the user — just exit cleanly.
      process.exit(1);
    } else {
      throw error;
    }
...
function startFigma() {
  const port = getCdpPort(); // resolved by detectCdpPort() or FIGMA_CDP_PORT env var
...
// figma-use requires Node.js 18.x due to an upstream ESM bug (Module.enableCompileCache
// is not available in ESM on Node 20+). Warn immediately so the user knows why
// commands will fail, rather than getting a cryptic error mid-run.
{
  const nodeMajor = parseInt(process.versions.node.split('.')[0], 10);
  if (nodeMajor !== 18) {
    console.warn(chalk.yellow(`\n⚠  figma-use requires Node.js 18.x (you are running ${process.version})`));
    console.warn(chalk.white('   Some commands may fail with an enableCompileCache error.'));
    console.warn(chalk.gray('   Fix: nvm install 18 && nvm use 18'));
    console.warn(chalk.gray('   Or:  https://nodejs.org/en/download  →  choose v18 LTS\n'));
  }
}
...
async function checkConnection() {
  // Auto-detect CDP port once per process (probes 9222-9229, respects FIGMA_CDP_PORT env var).
  await detectCdpPort();
